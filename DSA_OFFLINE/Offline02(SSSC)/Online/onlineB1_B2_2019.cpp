...
void Initialize(vector<double> &distance, vector<int> &parent, int V)
{
    for (int i = 0; i < V; i++)
    {
...
double SSSC(vector<vector<pair<int, double>>> &adj, int V, int E, int src, int des)
{
    vector<double> distance(101);
...
int main()
{
   ...
   // vector<int> delays(n + 1);
    ...
    //     adj[u1].push_back({v1, dis + delays[u1]});
    //     adj[v1].push_back({u1, dis + delays[v1]});
    // }
    // string src, des;
    // cin >> src >> des;
    ...
    int src,des;
    cin>>src>>des;
    cout<<1-SSSC(adj,n,m,src,des)<<endl;
