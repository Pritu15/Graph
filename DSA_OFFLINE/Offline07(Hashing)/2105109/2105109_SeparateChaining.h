...
public:
    ...
    SeparateChaining(long long n)
    {
        this->Tablesize = nearestPrime(n);
        this->InitialTableSize = Tablesize;
        this->HashFunctionCnt = 1;
        InitializeTable();
        // this->HashTable = new Node *[Tablesize + 1];
    }
    ...
    */
    ...
    void dummyInsert()
    {
        HashTable.resize(Tablesize); // Resize HashTable to Tablesize
        for (int i = 0; i < Tablesize; i++)
        {
            HashTable[i] = new Node();                         // Initialize each element with a new Node object
            HashTable[i]->H = new SeparateChaining(Tablesize); // Initialize H member variable of each Node
        }
    }
    void onlineInsert(string key1,string key2,long long value)
    {
        long long i1=myHash1(key1);
       // long long i2=myHash1(key2);
        HashTable[i1]->H->insert(key2,value);
        cout<<"Parchi"<<endl;
    }
    long long onlineSearch(string key1,string key2="")
    {
        long long i1=myHash1(key1);
        if(key2=="")
        {
            printInternal(HashTable[i1]->H->HashTable);

        }
       return  HashTable[i1]->H->find(key2);
    }
